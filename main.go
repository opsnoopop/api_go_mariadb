...
func (a *App) handleRoot(w http.ResponseWriter, r *http.Request) {
	...
	jsonWrite(w, http.StatusOK, map[string]string{"message": "Hello World from Go (MariaDB)"})
}
...
func (a *App) createUser(w http.ResponseWriter, r *http.Request) {
	...
	// 60s timeout คล้าย acquireTimeout/timeout ในต้นฉบับ
...
func main() {
	host := mustEnv("DB_HOST", "container_mysql")
	...
	// ตั้งค่า connection pool ให้คล้าย mysql2/promise
	db.SetMaxOpenConns(10)              // connectionLimit
	db.SetMaxIdleConns(10)
	db.SetConnMaxLifetime(30 * time.Minute)
	db.SetConnMaxIdleTime(10 * time.Minute)

	// ตรวจสุขภาพการเชื่อมต่อครั้งแรก
